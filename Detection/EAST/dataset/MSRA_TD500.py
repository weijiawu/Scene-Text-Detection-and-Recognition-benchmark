...
def get_ann(img, gt_path):
    ...
    for line in lines:
        ...
        bbox = np.array(adjust_box_sort(np.asarray(bbox.reshape(-1)))) / ([w * 1.0, h * 1.0] * 4)
        # bbox = bbox.reshape(-1)
...
def random_scale(img):
    ...
    base_scale = 512.0 / min(h, w)
    random_scale = np.array([0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4])
    scale = np.random.choice(random_scale) * base_scale
    img = scale_aligned(img, scale)
    return img

class MSRA_TD500(data.Dataset):
    def __init__(self, msra_train, hust_train, scale=0.25, length=512):
        super(MSRA_TD500, self).__init__()


        data_dirs = [os.path.join(msra_train,"Train_image"), hust_train]
        gt_dirs = [os.path.join(msra_train,"Train_gt"), hust_train]
...
if __name__ == "__main__":
    ...
    train_img_path = os.path.abspath('/data/data_weijiawu/TD500/')
    train_gt_path = os.path.abspath("/data/data_weijiawu/HUST-TR400/")

    trainset = MSRA_TD500(train_img_path,train_gt_path)
    ...
    for i, (img, gt_score, gt_geo, ignored_map) in enumerate(train_loader):
        ...
        gt_score = cv2.resize(gt_score, (512, 512))
        ...
        ignored_map_target_one = cv2.resize(ignored_map_target_one, (512, 512))
