...
def crop_img(img, vertices, labels, length,maxn_side=1280):
    ...
    # if h >= w and h > maxn_side:
    #     img = img.resize((int(w * maxn_side / h), maxn_side), Image.BILINEAR)
    # elif h < w and w > maxn_side:
    #     img = img.resize((maxn_side, int(h * maxn_side / w)), Image.BILINEAR)
...
def extract_vertices(img,lines):
    ...
    for line in lines:
        ...
        labels.append(label)
    return np.array(vertices,np.float), np.array(labels)
...
def random_scale(img):
    ...
    # max_scale = 2880.0 / max(h, w)
    # base_scale = max(base_scale, min_scale)
    # base_scale = min(base_scale, max_scale)
    base_scale = 640.0 / min(h, w)
    random_scale = np.array([0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.5, 1.6])
    scale = np.random.choice(random_scale) * base_scale
    img = scale_aligned(img, scale)
    return img

# def random_scale( im, text_polys, scales):
#     """
...
#     :return: 经过缩放的图片和文本
#     """
#     tmp_text_polys = np.array(text_polys).copy()
#     rd_scale = float(np.random.choice(scales))
#     print(int(im.width*rd_scale))
    # im = im.resize((int(im.width*rd_scale), int(im.height*rd_scale)), Image.BILINEAR)
    # tmp_text_polys *= rd_scale
    # return im, tmp_text_polys

class ICDAR17(data.Dataset):
    def __init__(self, img_path,gt_path, scale=0.25, length=640):
        super(ICDAR17, self).__init__()

        # self.scales = [0.5, 0.7, 0.9 , 1.0, 1.2, 1.4, 1.6 , 1.8 , 2.0]
        ...
        self.scale = scale
        self.length = length
        print("train sample:",len(self.img_files))
    ...
    def __getitem__(self, index):
        ...
        if vertices.shape[0] > 0:
            vertices = (vertices * ([img.width, img.height] * 4)).astype('int32')
            # vertices = np.reshape(vertices * ([img.width, img.height] * 4), (vertices.shape[0], int(vertices.shape[1] / 2), 2)).astype('int32')
...
if __name__ == "__main__":
    ...
    train_img_path = os.path.abspath('/data/data_weijiawu/ICDAR17/train_image/')
    ...
    for i, (img, gt_score, gt_geo, ignored_map) in enumerate(train_loader):
        ...
        gt_score_path = "/home/wwj/workspace/Sence_Text_detection/AAAI_EAST/Baseline/EAST_v1/show/gt_score.jpg"
        ignored_map_path = "/home/wwj/workspace/Sence_Text_detection/AAAI_EAST/Baseline/EAST_v1/show/ignored_map.jpg"
        image = img[0]
        ...
        img_show = image.permute(1, 2, 0).cpu().numpy()
        gt_score = (gt_score[0].permute(1, 2, 0).cpu().numpy()) * 255
        gt_score = cv2.resize(gt_score, (640, 640))
        cv2.imwrite(gt_score_path, gt_score)

        print(ignored_map.shape)
        ignored_map_target_one = ignored_map[0].permute(1, 2, 0).cpu().numpy() * 255
        ignored_map_target_one = cv2.resize(ignored_map_target_one, (640, 640))
        print(ignored_map_target_one.shape)
        cv2.imwrite(ignored_map_path, ignored_map_target_one)
        break