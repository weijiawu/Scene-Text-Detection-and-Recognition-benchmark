...
from dataset.ICDAR2013 import ICDAR13
...
# Model path
parser.add_argument('--exp_name',default= "ICDAR13", help='Where to store logs and models')
...
def test(epoch,  model, input_path,output_path,f_score):
    ...
    try:
        if f_score_new>f_score:
            ...
            torch.save(state_dict, os.path.join(args.workspace, 'best_model.pth'.format(epoch + 1)))
            f_score = f_score_new
    except:
        print(f_score_new)
...
if __name__ == '__main__':
    ...
    logger = setup_logger(os.path.join(args.workspace, 'train_icdar13_log'))
    ...
    #
    # target domain
    trainset = ICDAR13(args.train_data,args.train_gt)
