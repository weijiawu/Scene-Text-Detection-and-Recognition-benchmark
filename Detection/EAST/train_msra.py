...
from dataset.MSRA_TD500 import MSRA_TD500
...
parser.add_argument('--msra_data', default='/data/data_weijiawu/TD500/', type=str,
                    help='the path of training data ')
parser.add_argument('--hust_path', default="/data/data_weijiawu/HUST-TR400/", type=str,
...
# Training strategy
parser.add_argument('--epoch_iter', default=400, type = int,
...
def test(epoch,  model, input_path,output_path,f_score):
    model.eval()
    output_path = os.path.join(output_path,"submit")
    os.makedirs(output_path, exist_ok=True)
    input_path = os.path.join(input_path,"Test_image")
    ...
    for one_image in tqdm(image_list):
        ...
        boxes = detect_msra(img, model, device)
...
if __name__ == '__main__':
    ...
    trainset = MSRA_TD500(args.msra_data,args.hust_path)
    ...
    for epoch in range(args.epoch_iter):
        ...
        #     state_dict = model.module.state_dict() if data_parallel else model.state_dict()
        #     torch.save(state_dict, os.path.join(args.workspace, '{}_model.pth'.format(epoch + 1)))
        # 进行target domain的eval，看看指标。
        if epoch>120:
            f_score = test(epoch, model, args.msra_data, args.workspace, f_score)
