...
def image_label(im_fn: str, text_polys: np.ndarray, text_tags: list, n: int, m: float, input_size: int,
                defrees: int = 10,
                scales: np.ndarray = np.array([0.5, 0.8, 1.0, 1.3,1.8, 2.0, 3.0])) -> tuple:
    '''
...
class ICDAR15(data.Dataset):
    ...
    def load_data(self, data_dir: str) -> list:
        data_list = []
        for x in glob.glob(data_dir + '/train_image/*.jpg', recursive=True):
            d = pathlib.Path(x)
            label_path = os.path.join(data_dir,'train_gt', ('gt_' + str(d.stem) + '.txt'))
...
if __name__ == '__main__':
    ...
    train_data = ICDAR15(config.trainroot, data_shape=config.data_shape, n=config.n, m=config.m,
