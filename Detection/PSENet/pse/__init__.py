...
def decode_icdar17(preds, scale,org_img,is_test, threshold=0.7):
    """
    ...
    pred, label_values = pse_warpper(preds, 2)
    ...
    for label_value in label_values:
        ...
        if is_test:
            bbox_list.append([bbox[1][0],bbox[1][1], bbox[2][0],bbox[2][1], bbox[3][0],bbox[3][1], bbox[0][0],bbox[0][1], np.array(1.0).astype(float)])
        else:
...
def decode_total(preds, scale,org_img,kern_size_, threshold=0.5):
    """
    在输出上使用sigmoid 将值转换为置信度，并使用阈值来进行文字和背景的区分
    :param preds: 网络输出
    :param scale: 网络的scale
    ...
    for label_value in label_values:
        ...
        kernel = np.ones((kern_size_, kern_size_), np.uint8)
